...
import {
  ArrowLeft, RefreshCw, Search, CheckCircle,
...
export function M365FindingsPlatformPage() {
  ...
  const provider = FINDING_PROVIDERS.find(p => p.connectorId === 'm365')

  const [loading, setLoading]       = useState(true)
  const [findings, setFindings]     = useState([])   // normalised finding objects
  const [sevFilter, setSevFilter]   = useState('all')
  const [catFilter, setCatFilter]   = useState('all')
  const [search, setSearch]         = useState('')
  const [modal, setModal]           = useState(null)  // { type: 'risk'|'incident', finding }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const normalised = rows.flatMap(r => provider.derive(r))
      setFindings(normalised)
    } catch (e) {
      console.error('[M365Findings] load error', e)
    } finally {
      setLoading(false)
    }
  }, [organization?.id])
  ...
  const categories = useMemo(() => {
    const seen = new Set(findings.map(f => f.subject.meta))
    return ['all', ...seen]
  }, [findings])
  ...
  const visible = useMemo(() => findings.filter(f => {
    ...
    if (q) {
      const hay = `${f.title} ${f.subject.name || ''} ${f.description}`.toLowerCase()
        ...
        {loading ? (
          ...
          <div className="rounded-xl py-14 text-center" style={{ background: '#fff', border: '1px dashed #e5e0e0' }}>
            ...
            <p className="text-sm font-medium mb-1" style={{ color: '#4a3a3a' }}>
              {findings.length === 0 ? 'No M365 findings yet' : 'No findings match this filter'}
            </p>
            <p className="text-xs" style={{ color: '#8a7070' }}>
              {findings.length === 0
                ? 'Run a scan from M365 Security settings to pull findings.'
                : 'Try a different severity level or category.'}
            </p>
            {findings.length === 0 && (
              <button onClick={() => navigate('/app/settings/m365')}
          ...
          <div className="flex flex-col gap-3">
            {visible.map(finding => {
              const catMeta = CATEGORY_META[finding.subject.meta] || { label: finding.subject.meta, Icon: ShieldAlert, color: '#5D0F0F' }
              return (
                <div key={finding.key}
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    <CategoryIcon category={finding.subject.meta} />
